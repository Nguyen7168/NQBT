...
def visualize(output_folder_path, image_path, anomaly_map_image):
    ...
    cv2.imwrite(heat_map_save_path, cv2.cvtColor(heat_map_image, cv2.COLOR_RGB2BGR))

    mask_save_path = os.path.join(output_folder_path, f"mask_{os.path.basename(image_path)}")
    mask = (heat_map_resized * 255).astype(np.uint8)  
    cv2.imwrite(mask_save_path, mask)
    
    binary_mask_save_path = os.path.join(output_folder_path, f"binary_mask_{os.path.basename(image_path)}")
    binary_mask = np.where(heat_map_resized > 0.2, 255, 0).astype(np.uint8)
...
def main_process(image_folder_path, output_folder_path, onnx_model_path, input_size, max_ratio, visualize_output):
    ...
    binary_masks_folder = os.path.join(output_folder_path, "binary_masks")
    ...
    for idx, file in enumerate(all_files):
        ...
        binary_mask = np.where(anomaly_map_image > 0.2, 255, 0).astype(np.uint8)
        binary_mask = cv2.resize(binary_mask, (800, 800), interpolation=cv2.INTER_NEAREST)

        binary_mask_path = os.path.join(binary_masks_folder, f"img_{idx}_mask.png")
        cv2.imwrite(binary_mask_path, binary_mask)

        img_path = os.path.join(binary_masks_folder, f"img_{idx}.png")
        original_img = cv2.imread(image_path)
        original_img_resized = cv2.resize(original_img, (800, 800), interpolation=cv2.INTER_LINEAR)
        cv2.imwrite(img_path, original_img_resized)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--image_folder_path', type=str, default="./Input_INP", help='Path to input image folder')
    parser.add_argument('--output_folder_path', type=str, default="./Onput_INP", help='Path to save visualized outputs')
    parser.add_argument('--onnx_model_path', type=str, default=r"D:\AI\yolov8\INP-model\model_664_0.1533680558204651.onnx", help='Path to the ONNX model file')
    parser.add_argument('--input_size', type=int, default=448, help='Input size for model inference')
    parser.add_argument('--max_ratio', type=float, default=0.02, help='Max ratio used for score thresholding')
