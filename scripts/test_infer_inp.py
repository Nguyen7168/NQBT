...
def main() -> int:
    parser = argparse.ArgumentParser(description="Test INP anomaly inference")
    ...
    base_out = Path(args.out or cfg.io.output_dir) / f"test_inp_{ts}"
    ensure_dir(base_out)

    thres = float(cfg.models.inp.inp_threshold)
    bin_th = float(cfg.models.inp.inp_bin_thresh)
    ...
    for img_path in args.images:
        ...
        print(f"Saved INP results for {p} -> {out_dir} | score={score:.4f} ({status})")
